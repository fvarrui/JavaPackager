 ...
 * This is used internally to share the parameter validation logic between the
 * Ant task and the CLI tool.
 *
 * @since 2.0
 */
public class SignerHelper {
	...
	private AuthenticodeSigner build() throws SignerException {
		...
		if ("PKCS11".equals(storetype)) {
			// the keystore parameter is either the provider name or the SunPKCS11
			// configuration file
		...
		if (keystore != null) {
			...
			if (chain == null) {
				throw new SignerException("No certificate found under the alias '" + alias + "' in the keystore "
						+ (provider != null ? provider.getName() : keystore) + " (available aliases: "
						+ String.join(", ", aliases) + ")");
			}
			if (certfile != null) {
				if (chain.length != 1) {
				...
				try {
					...
					if (chainFromFile[0].equals(chain[0])) {
						...
						throw new SignerException(
								"The certificate chain in " + certfile + " does not match the chain from the keystore");
					}
		...
		return new AuthenticodeSigner(chain, privateKey)
				.withProgramName(name).withProgramURL(url)
	 ...
	 *
	 * @param proxyUrl      the url of the proxy (either as hostname:port or
	...
	private void initializeProxy(String proxyUrl, final String proxyUser, final String proxyPassword)
		...
		if (proxyUrl != null && proxyUrl.trim().length() > 0) {
			...
			final URL url = new URI(proxyUrl).toURL();
